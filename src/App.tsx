...
const faultlineLabels: Record<ConflictAnalysis['faultlines'][number]['type'], string> = {
  FACT: 'Needs evidence',
  VALUE: 'Different priorities',
  UNKNOWN: 'Not known yet',
  DEFINITION: 'Needs a shared definition',
}
      ...
      <main id="top">
        ...
        <section className="workspace container">
          ...
          <section className={`results ${view}`} aria-live="polite">
            ...
            <div className="shared-strip understand-content"><div className="shared-title"><span className="shared-icon">∩</span><div><span className="mini-label">WHAT THEY AGREE ON</span><strong>There is more common ground than it feels like.</strong></div></div><div className="shared-items">{analysis.sharedGround.map((item) => <span key={item}>{item}</span>)}</div></div>

            <div className="section-heading understand-content"><div><span className="step">03</span><h2>What needs proof?</h2></div><button className="icon-button" title="What do these labels mean?"><CircleHelp size={17} /></button></div>
            <div className="faultline-grid understand-content">{analysis.faultlines.map((line) => <article className={`faultline ${line.type.toLowerCase()}`} key={`${line.type}-${line.title}`}><span className="fault-label">{faultlineLabels[line.type]}</span><h3>{line.title}</h3><p>{line.explanation}</p>{line.missingEvidence && <div className="missing-evidence"><span>What to check</span>{line.missingEvidence}</div>}</article>)}</div>

            <section className="section-heading pressure-heading decide-content"><div><span className="step">03A</span><h2>What could go wrong?</h2></div><ShieldAlert size={18} /></section>
            <section className="red-team-panel decide-content"><div className="red-team-intro"><ShieldAlert size={19} /><p>Check the weak spot before the team commits.</p></div><div className="pressure-grid"><article><span>The strongest case against this</span><p>{analysis.redTeam.strongestCounterargument}</p></article><article><span>The risky assumption</span><p>{analysis.redTeam.hiddenAssumption}</p></article><article><span>What would change the decision</span><p>{analysis.redTeam.evidenceThatWouldChangeMind}</p></article><article><span>The first small test</span><p>{analysis.redTeam.preCommitmentTest}</p></article></div></section>

            <div className="resolution-panel decide-content"><div className="resolution-intro"><div className="resolution-icon"><Lightbulb size={21} /></div><div><span className="mini-label">THE PRACTICAL NEXT STEP</span><h2>{analysis.resolution.title}</h2><p>{analysis.resolution.rationale}</p></div></div><div className="resolution-steps">{analysis.resolution.steps.map((step, index) => <div className="resolution-step" key={step}><span>{String(index + 1).padStart(2, '0')}</span><strong>{step}</strong></div>)}</div><div className="criteria-block"><span className="mini-label">HOW WE'LL KNOW IT WORKED</span><ul>{analysis.resolution.successCriteria.map((criterion) => <li key={criterion}>{criterion}</li>)}</ul></div><div className="executor-fields"><label><span><UserRound size={13} /> Who owns this?</span><input value={owner} onChange={(event) => setOwner(event.target.value)} placeholder="Name the person" /></label><label><span><CalendarDays size={13} /> When do we check?</span><input type="date" value={reviewDate} onChange={(event) => setReviewDate(event.target.value)} /></label></div><div className="resolution-actions"><button className="outline-button" onClick={() => setPracticeOpen(true)}><Play size={15} /> Practice this resolution</button><button className="outline-button" onClick={copyDecisionPacket}><Clipboard size={15} /> {copiedUpdate ? 'Decision packet copied' : 'Copy decision packet'}</button></div></div>
